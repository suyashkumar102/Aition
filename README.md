# Aition — Causal AI Fairness Engine
...
Aition is the first tool to make this tension interactive and actionable. It forces an explicit, informed organisational choice instead of hiding the decision inside a metric.
...
| Metric | Standard Audit (AIF360) | Aition Causal Audit |
|---|---|---|
...
```bash
# Clone and install
git clone https://github.com/yourteam/Aition
cd Aition
...
A set of variables Z satisfies the backdoor criterion relative to (X, Y) if Z blocks all backdoor paths from X to Y and no variable in Z is a descendant of X. Aition uses this to identify which intermediate variables are carrying causal influence from protected attributes to outcomes — the proxy discrimination paths.
...
Constraint-based causal discovery algorithm that learns causal graph structure from observational data by testing conditional independence relationships. Aition uses DoWhy's implementation with Fisher's Z-test for continuous variables.
...
India's Personal Data Protection Bill Section 12(b) introduces algorithmic accountability requirements for automated decision-making systems. Every Aition audit produces an immutable record of the causal paths found, the fairness metrics computed, and the debiasing strategy applied — the documentation trail that PDPB compliance will require.
...
│   ├── public/
│   └── package.json
├── Aition_Architecture_DesignSpec.md
└── Aition_Round1_MasterPlan.md
```
